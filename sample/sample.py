...
def sample(node: TREParser.ExprContext, n, T, vol_cache):
    ...
    match node_type:
        ...
        case TREParser.ParenExprContext:
            ...
            e0 = node.expr()
            return sample(e0, n, T, vol_cache)
        ...
        case TREParser.UnionExprContext:
            ...
            vol1 = slice_volume(e1, n, cache=vol_cache)
            vol2 = slice_volume(e2, n, cache=vol_cache)
            ...
            return sample(chosen_expr,n,T,vol_cache)
        ...
        case TREParser.TimedExprContext:
            ...
            return sample(sub, n, T, vol_cache)
        ...
        case TREParser.ConcatExprContext:
            ...
            return concat_sampling(node, e1, e2, n, T, vol_cache)
...
def concat_sampling(node, e1:TREParser.ExprContext, e2:TREParser.ExprContext, n, T, vol_cache):
    """
    ...
    concat_vol = slice_volume(node, n, cache=vol_cache)
    ...
    k = sample_k(n, T, concat_vol, e1, e2, vol_cache)

    # this never recomputes anything
    vol1:VolumePoly = slice_volume(e1, k, cache=vol_cache)
    vol2:VolumePoly = slice_volume(e2, n-k, cache=vol_cache)
    ...
    # in the delta case we directly call the sampler for the child that isn't delta
        ...
        return sample(e2, n-k, T, vol_cache)
    elif vol2.delta and not vol2.intervals:
        return sample(e1, k, T, vol_cache)
    ...
    # TODO this is not a pdf... is that normal? I thought that we divide one smaller volume by another, and if we divide till infty it will be all volume dividwed by all volume?

    # assert pdf.total_volume() == 1, "Problem in sampling of T in a concatenation, this should be a pdf. This is most likely a bug."

    # TODO sample this pdfto get T1, T2 for subexpressions -> should be doable with my integral function
    T1 = pdf.inverse_sampling()
    T2 = T-T1

    return sample(e1,k,T1,vol_cache) * sample(e2, n-k, T2, vol_cache)

def sample_k(n, T, concat_vol, e1, e2, vol_cache) -> int:
    """
    ...
    for k in range(n+1):
        ...
        # this never recomputes anything
        v1 = slice_volume(e1, k, cache=vol_cache)
        v2 = slice_volume(e2, n-k, cache=vol_cache)

        # This function desribes the volumes of words w1w2 where l(w1) = k and l(w2) = n-k.
        # TODO these could be cached as well in slice_volume, with different keys?
        V_k = v1 ** v2
    ...
    # assert np.sum(probas) == 1, "Problem during sampling of k. Invalid distribution."

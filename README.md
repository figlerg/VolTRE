...
This repository is the EMSOFT 2026 artifact for the paper *"Uniform Sampling for Timed Regular Expressions"* (accepted at EMSOFT 2026). **Reviewers start here.** See [INSTALL.md](INSTALL.md) (setup, Docker), [REQUIREMENTS.md](REQUIREMENTS.md), and [STATUS.md](STATUS.md) (badges). Quick check of the installation: `./artifact/smoke_test.sh`.
...
Each experiment script writes its raw output into `artifact/output/intermediates/`. Only the final deliverables are copied up into `artifact/output/` under `figN_*` names matching the figure numbers in the paper, so `ls artifact/output/` shows just those figures (plus the `intermediates/` folder for anyone who wants the underlying PDFs, CSVs, and `.dat` files) and a reviewer can match outputs to figures at a glance.

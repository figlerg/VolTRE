...
Each run writes the script's own output file(s) into `artifact/output/`, then copies them to `figN_*` names matching the figure numbers in the paper (so a reviewer can match outputs to figures at a glance).

...
| `reproduce.sh` name | paper figure (PDF name) | fast | `--full` |
|---|---|---|---|
| `deltasigma` | `08*_delta_sigma_*_n_10.pdf` (3 files) | ~2 min | same (always computed live) |
| `maxent` | `exp15_maxent_triangle_variance_3_cropped.pdf` | seconds | ~3 min |
| `sharkfin` | `sharkfin.pdf` | seconds | ~40 min |
| `stress` | `11_stress_ex123_3.pdf` | seconds | ~10 min |
| `ksweep` | `exp16_ksweep_v8_k8.pdf` | seconds | ~30 min, needs wordgen |

Full-mode times measured in the Docker container on the reference machine (see REQUIREMENTS.md), ~1.5 h in total. `ksweep --full` can take up to several hours on machines where wordgen runs into the 1 h timeout instead of failing fast on memory.

Fast mode (default) rebuilds the plots from the committed measurement data and reproduces the paper figures exactly. `--full` recomputes all measurements from scratch (seed 42); for the two timing figures (`stress`, `ksweep`) absolute numbers depend on your hardware while the qualitative result is reproduced. Without a LaTeX installation the figures fall back to matplotlib's mathtext (content identical, fonts differ); the Docker image ships LaTeX and the wordgen baseline needed for `ksweep --full`.

...
The reviewer-facing rework of the eval loop (decided with Felix 2026-07-22) is DONE in the sandbox, pending Felix's diff review + a full Docker re-verify:
1. [DONE] fig:cube (Figure 2) added — source folder experiments/paper_experiments/fig2_cube/ (Benoit's wordgen data + gnuplot 3D/projection + VolTRE volume). Fast (committed .dat) and full (--resample via wordgen). Adds gnuplot-nox as a Docker dependency (done).
2. [DONE] figN naming + full uniform rewrite of the runners. Every reproducible figure is now one real experiment script taking plain CLI flags; reproduce.sh is a flat list of direct calls + `cp` to figN names (fig2a/2b/2c, fig3, fig4, fig6, fig7, fig9a/9b/9c). Retrofitted argparse into theorem4.py, exp15_maxent_triangle.py, exp16_ksweep.py, make_combined_plot.py (VOLTRE_* env-var contract removed). Added committed stress_compute.py (kills the notebook-cell-surgery). Added standalone make_delta_sigma_fig.py next to its .tre specs. Removed all six artifact/figures/*.py wrappers. Full fast reproduce loop verified end to end in the sandbox: all 10 figN files produced, exit 0. Follow-up 2026-07-22 (Felix saw the Docker output dir had paper-named intermediates + figN copies): reproduce.sh now routes every script's output into artifact/output/intermediates/ (persistent, wiped at the start of every run) and copies ONLY the figN_* files up into artifact/output/. So `ls artifact/output/` shows the 10 figN deliverables plus one intermediates/ folder (holding the paper-named PDFs and the fresh --full CSVs for anyone who wants to dig deeper). Superseded an earlier mktemp-scratch version after Felix asked for the supporting files to persist and be easy to see. Re-verified in the sandbox. README also restructured 2026-07-22: the whole "EMSOFT 2026 Artifact" section now leads, placed right after the intro blurb (before Contributors/Thesis/Paper), reworded to stand alone as the reviewer entry point ("Reviewers start here") since reviewers skim rather than read the full README.
3. [DONE] Reproducibility map table added to STATUS.md (paper fig -> output figN -> script -> reproducible?, all 10 figures incl. non-reproducible ones for completeness). README table updated (cube row, figN column, paper fig numbers). Dockerfile gets gnuplot-nox.
4. [NEXT] Commit this refactor block, then Felix runs a full Docker re-verify (fast + --full, ~85 min) on a real machine to confirm nothing regressed.
5. THEN archival (section E): cut emsoft26-artifact branch + tag, Zenodo DOI (then fill STATUS placeholder), paper PDF into archive, clean-machine vetting, HotCRP form. Watch the ΣΔ Simulink discussion.

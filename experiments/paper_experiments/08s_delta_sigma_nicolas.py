...
import numpy as np
import random
from unpickle_and_translate import convert_pickles
...
2nd version:                (<b>_[3,4]*) . (< <a>_[1,2] . ( <b>_[3,4] . (<b>_[3,4]*) )             )*
another spec:               (<a>_[1,2] + <b>_[3,4])*
"""
...
match spec_case:
    case 'original':
        figname = "08a_delta_sigma_nicolas"
        spec = "08a_nicolas_spec_5_inf.tre"
    ...
    case 'fat_thin':
        figname = "08b_delta_sigma_nicolas_fat_thin"
        spec = "08b_nicolas_spec_fat_thin.tre"
        title = r"$({\langle b\rangle _{[3,4]}}^*) \cdot (  \langle a\rangle _{[1,2]} \cdot ( \langle b\rangle _{[3,4]} . (\langle b\rangle _{[3,4]}^*) ) )^*$"
    case 'other':
        figname = "08c_delta_sigma_thao"
        spec = "08c_thao_spec.tre"
        title = r"$(   \langle a\rangle _{[1,2]}    {\langle b\rangle} _{[3,4]}) ^*  $"
...
def experiment():
    fig, axs = plt.subplots(1, 1, figsize=(fig_width_in, fig_height_in*0.55))  # 2x1 grid layout
    ctx = quickparse(spec)
